...
def process_pframe(
        ...
        for bx in range(blocks_horizontal):
            ...
            while step > 1:
                ...
                for p in points:
                    ...
                    window = search_area[box[0][1] : box[1][1], box[0][0] : box[1][0]]
                    ...
                    if error < min_mse:
                        ...
                        best_residual = target_block.astype(np.int16) - window.astype(
                            np.int16
                        )
                        p1 = p
                step = step / 2
    ...
    for by in range(blocks_vertical):
        for bx in range(blocks_horizontal):
            ...
            reconstructed_block = best_match_block.astype(np.int16) + residual.astype(
                np.int16
            )
            reconstructed_block = np.clip(reconstructed_block, 0, 255).astype("uint8")
