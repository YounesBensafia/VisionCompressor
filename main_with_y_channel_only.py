...
# Load the images
source_image_path = "image1.png"
target_image_path = "image2.png"
    ...
    for bx in range(blocks_horizontal):
        ...
        while step > 1:
            ...
            for p in points:
                ...
                if error < min_mse:
                    ...
                    best_residual = target_y.astype(np.int16) - window.astype(np.int16)
                    p1 = p
            step = step / 2
        ...
        if best_match_position is not None:
            ...
            if min_mse < threshold:
                ...
                residual_image[y : y + block_size, x : x + block_size, 0] = 0
            else:
                # Store the residual
...
residual_display = np.abs(residual_image).astype(np.uint8)  # Normalize for display
...
for by in range(blocks_vertical):
    for bx in range(blocks_horizontal):
        ...
        residual = residual_image[y : y + block_size, x : x + block_size, 0]
        reconstructed_y = best_match_y.astype(np.int16) + residual
        reconstructed_y = np.clip(reconstructed_y, 0, 255).astype("uint8")

        reconstructed_block = cv2.merge(
            (reconstructed_y, reconstructed_y, reconstructed_y)
        )
